...
$(document).ready(function () {
  ...
  $("#claimStatusDropdown .dropdown-item").click(function (e) {
    ...
    $.ajax({
      type: "POST",
      url: fileLocation + "../../src/process.php",
  ...
  $("#tripNumber, #fbNumber, #cases, #productNumber, #trailerNumber").change(
    function () {
  ...
  if ($("#claimsTable")) {
    var claimsTable = $("#claimsTable").DataTable({
      responsive: true,
      pageLength: 25,
      ajax: fileLocation + "../../src/api.php?action=list",
      language: {
        paginate: {
    ...
    $("#claimsTable tbody").on("click", "td.dt-edit", function () {
      ...
      window.location.replace("src/views/details.php?id=" + row.id)
    })
  }
  ...
  if ($("#driverClaimsTable")) {
    ...
    $("#driverClaimsTable tbody").on("click", "td.dt-edit", function () {
      ...
      window.location.replace("src/views/details.php?id=" + row[1])
    })
  }
